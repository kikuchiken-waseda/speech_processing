...
確率変数と確率分布
==================================
確率変数とは、統計学の確率論において、起こりうる事柄に割り当てている値を取る変数。
確率分布とは、確率変数に対して、各々の値をとる確率(または尤度)を表したものである。

例えば「サイコロを振る」という事象を考えた際に確率変数はサイコロの数字である :math:`x={1,2,3,4,5,6}` となる。
この時の確率分布 :math:`p(x)` は
...
.. math::
    \underset{G}{\min}\underset{D}{\max} V(D,G) = \mathbb{E}_{p_r(\mathbf{x})}[\log D(\mathbf{x})] + \mathbb{E}_{p_z(\mathbf{z})}[\log (1-D(G(\mathbf{z})))]
